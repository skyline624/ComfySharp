"""Prospective text predicates and exhaustive lower digests. Preflight executes no source."""
import argparse
import asyncio
import contextlib
import copy
...
PROTOCOL_SHA256 = "0d00fa0c7721b07b2fc12381f60ed364b286963553577fcdb6672c028e18aa92"
ARGUMENTS = {"StringContains": ("string", "substring", "case_sensitive"),
             "StringCompare": ("string_a", "string_b", "mode", "case_sensitive")}
...
def load_protocol():
    ...
    if value["schema"] != 1 or value["id"] != "text-comparison-python312-v1":
    ...
    if {k: sum(c["portClassification"] == k for c in cases) for k in
            ...
            "comparable": 36, "source-error": 2, "source-only-invalid-mode": 2}:
        raise ValueError("Unexpected prospective partition")
    for case in cases:
        if case["node"] not in ARGUMENTS or tuple(case["promptInputs"]) != ARGUMENTS[case["node"]]:
            raise ValueError("Unexpected node or argument order")
    lower = value["helperLower"]
    if len(lower["planes"]) != 17 or len(lower["modes"]) != 4 or lower["repeats"] != 3:
...
def load_helper(protocol):
    verify_helpers(protocol)  # All three raw files checked before importing the driver.
    spec = importlib.util.spec_from_file_location(
        "_text_comparison_prefix_driver", ROOT / "labs/autogrow-source/reference.py")
...
def source_modules(helper, compiled):
    ...
    with helper.source_modules(compiled) as (io, execution, _, blocker):
        name = "_text_comparison_source_nodes"
...
async def run_case(helper, compiled, case, observe):
    result = {"id": case["id"], "node": case["node"], "portClassification": case["portClassification"],
              "bodyOrigin": "frozen " + case["node"] + ".execute -> CPython builtin predicates/lower"}
        ...
        for mode in plan["modes"]:
            ...
            for _ in range(plan["repeats"]):
                ...
                for cp in range(plane["startInclusive"], plane["endInclusive"] + 1):
                    ...
                    source, lowered = framed(cp, text), framed(cp, text.lower())
            ...
            if repeats[0] != repeats[1] or repeats[1] != repeats[2]:
                raise AssertionError("Builtin lower repeats differ")
            records.append(dict(plane=plane["plane"], mode=mode["id"], **repeats[0],
                                repeatSha256=[r["sha256"] for r in repeats], repeatsIdentical=True))
    if len(records) != 68:
        raise AssertionError("Expected 68 lower digests")
    return {"id": plan["id"], "framing": plan["framing"], "records": records,
            "totalValidScalars": plan["totalValidScalars"], "inputApplicationsPerPass": 4 * plan["totalValidScalars"],
...
def public_evidence(admitted, execute):
    ...
    for name in PUBLIC_FILES:
        relative = "labs/text-comparison-source/" + name
...
def main():
    ...
    evidence = dict(schema=1, protocolId=protocol["id"], protocolSha256=PROTOCOL_SHA256,
                    ...
                    laboratoryFiles=public, cases=len(protocol["cases"]), lowerDigestPlans=68, executed=False,
    ...
    evidence.update(executed=True, collectorCommit=admitted, python=platform.python_version(),
                    ...
                    helperLower=lower, limitations=[protocol["scope"], protocol["comparison"], protocol["fixtureEncoding"],
                    ...
                    "68 lower digests do not exhaust every multi-character Unicode context."])
    ...
    with (destination / "reference.json").open("xb") as stream:
        stream.write(raw)
    print(json.dumps(dict(cases=len(records), lowerDigests=68, bytes=len(raw), sha256=digest(raw))))
