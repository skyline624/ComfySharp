...
import argparse,ast,functools,hashlib,json,logging,math,pathlib,struct,subprocess,types
...
algorithm=report.get('algorithm','LoRA');assert algorithm in ['LoRA','LoHa']
expected_tensors=1814 if algorithm=='LoHa' else 1250
...
for name,info in header.items():
 ...
 suffix=next(s for s in ['.lora_up.weight','.lora_down.weight','.hada_w1_a','.hada_w1_b','.hada_w2_a','.hada_w2_b','.hada_t1','.hada_t2','.alpha','.diff_b','.diff'] if name.endswith(s))
 stem=name[:-len(suffix)];mapping[stem]=stem.removeprefix('diffusion_model.')+'.weight'
assert cursor==len(body) and len(tensors)==expected_tensors and set(tensors)==set(e['parameterHashes'])
...
ns['weight_adapter']=types.SimpleNamespace(adapters=[ns['LoRAAdapter'],ns['LoHaAdapter']],WeightAdapterBase=ns['WeightAdapterBase'])
...
kind='loha' if algorithm=='LoHa' else 'lora';adapter_class=ns['LoHaAdapter' if algorithm=='LoHa' else 'LoRAAdapter']
counts={kind:0,'weightDifferences':0,'biasDifferences':0}
for name,patch in loaded.items():
 if isinstance(patch,adapter_class):counts[kind]+=1
 else:
...
assert counts=={kind:282,'weightDifferences':109,'biasDifferences':295}
with open(a.output,'x',encoding='utf-8',newline='\n') as f:
 json.dump({'status':'ok','sourceCommit':commit,'sourceHashes':hashes,'adapterSha256':e['adapterSha256'],'payloadCount':expected_tensors,'allPayloadHashesMatch':True,'sourceAcceptedTargets':len(loaded),'kinds':counts,'sourceAdditiveApplicationsExact':True,'scope':'Restricted independent F32 byte decoder, actual frozen mixed loader and additive application. Does not claim safetensors-package, pretrained source-forward/gradient or full training-node qualification.'},f,indent=2);f.write('\n')
print('All',expected_tensors,'parameter hashes match; frozen source accepts 686 mixed targets.')
