...
The first [three-platform campaign for `7426150`](https://github.com/skyline624/ComfySharp/actions/runs/34623298896) passes on Windows. Linux and macOS pass the routine and reduced reference tests, but both stock encoders exceed the same fixed numerical tolerance. Those platforms remain unqualified for this encoder profile. Diagnostic traces are being collected before deciding whether the difference comes from another port operation or from native backend arithmetic. The Windows reference is retained, no failing assertion is skipped and no tolerance is relaxed.
