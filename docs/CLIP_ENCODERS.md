...
The later [isolated source 2.10 comparison](qualification/clip-source-210-d43ccfb.md) finds bit-identical final, all-layer and raw pooled outputs between product and source on each OS, with projected pooling within the same bound. Source-versus-source comparisons reproduce the original Linux/macOS differences. This is evidence for the tested inputs, not a replacement of the existing acceptance profile; the wheels' and NuGet packages' CPU-compute libraries have distinct hashes.
