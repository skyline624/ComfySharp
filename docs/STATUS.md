...
|---|---|---|
| Dépôt/socle | Lot 0 validé : dépôt public indépendant, clone propre vérifié, .NET 10, verrous, GPLv3 ; 338 tests, probe CPU et démarrage graphique Desktop/Host sur les trois OS | Installations propres et distributions finales à qualifier au lot 12 |
...
| Calcul | Bruit natif par générateur, primitive Euler, cinq générateurs de sigmas CPU/F32 comparés à un corpus source, opérations et gradients CPU/CUDA réels | Nœuds SIGMAS pas encore intégrés au Host ; pas de parcours SD1.5 ni modèle entraîné ou génératif exécuté |
