...
Le socle contient un éditeur C#/XAML avec canvas de nœuds, un Host .NET séparé, des documents JSON conservés sans perte et un moteur avec valeurs natives. Son registre fournit **48 identifiants**, dont les utilitaires, les traitements SIGMAS/IMAGE, le [parcours SD1.5](docs/SD15_WORKFLOW.md) et les [nœuds LoRA](docs/LORA_NODES.md). Les [aperçus PNG natifs](docs/BITMAP_PREVIEWS.md) offrent une navigation dans les lots et conservent le workflow soumis dans les métadonnées. Les [workflows incorporés aux PNG](docs/PNG_WORKFLOW_IMPORT.md) peuvent être rouverts comme documents. Les graphes CLIP-L/G, U-Net SD1/SD2 et VAE classique sont implémentés dans la bibliothèque d'inférence ; la qualification complète des variantes et des plateformes reste en cours.
