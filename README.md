...
**État : développement initial 0.1.0-dev. Un parcours SD1.5 natif fonctionne avec des poids locaux, y compris dans le Host et l'éditeur ; aucune famille complète n'est encore qualifiée.** La vidéo, l'audio et la 3D restent à porter. Le périmètre final reste le catalogue local complet, y compris l'entraînement intégré. Voir [l'objectif](docs/OBJECTIF.md), [le plan approuvé](docs/MIGRATION.md), [l'avancement réel](docs/STATUS.md) et [la matrice](docs/capabilities/README.md).

Les [prompts API JSON ou incorporés aux PNG](docs/API_PROMPT_IMPORT.md) peuvent maintenant être importés en documents éditables. Les champs inconnus sont conservés ; l'exécution reste limitée aux nœuds réellement disponibles dans le Host.

Le socle contient un éditeur C#/XAML avec canvas de nœuds, un Host .NET séparé, des documents JSON conservés sans perte et un moteur avec valeurs natives. Son registre fournit **50 identifiants**, dont les utilitaires, les traitements SIGMAS/IMAGE, le [parcours SD1.5](docs/SD15_WORKFLOW.md), les [nœuds LoRA](docs/LORA_NODES.md), [SaveLoRA](docs/SAVE_LORA.md) et [LoraModelLoader](docs/LORA_MEMORY_LOADING.md) en modes ordinaire et [bypass SD Float32](docs/LORA_BYPASS.md). Le producteur d'adapter du parcours d'entraînement public reste à porter. Les [aperçus PNG natifs](docs/BITMAP_PREVIEWS.md) offrent une navigation dans les lots et conservent le workflow soumis dans les métadonnées. Les [workflows incorporés aux PNG](docs/PNG_WORKFLOW_IMPORT.md) peuvent être rouverts comme documents. Les graphes CLIP-L/G, U-Net SD1/SD2 et VAE classique sont implémentés dans la bibliothèque d'inférence ; la qualification complète des variantes et des plateformes reste en cours.
