...
/// <summary>Independently disposable owner of a complete classical encoder/decoder/quant parameter bank.</summary>
public sealed class ClassicalVaeWeightSet : IDisposable
{
    ...
    public static ClassicalVaeWeightSet FromOwnedTensors(ClassicalVaeConfig config, IReadOnlyDictionary<string, torch.Tensor> tensors)
        => new(config, CpuModelWeightBank.Create(ClassicalVaeWeightSchema.Describe(config), tensors));
    public ClassicalVaeWeightSet Retain() => new(Config, bank.Retain());
