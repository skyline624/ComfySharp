...
internal static class SdNativeAlignedSuiteDiagnostic
{
    private const string Variable = "COMFYSHARP_SD_ALIGNED_SUITE_TRACE_DIR";
        ...
        JsonElement reference, JsonElement parameterReferences, JsonElement config)
    {
        ...
        using var prepared = new SdAlignedDiagnosticInputs(inputs);
        latent = prepared.Values["latent"]; timesteps = prepared.Values["timesteps"]; context = prepared.Values["context"];
        inputs = prepared.Values;
        try
        {
            Run(directory, reference.GetProperty("id").GetString()!, "unet", bank, parameterReferences, config,
                new { }, inputs, reference.GetProperty("output"), captures, prepared,
                repeat =>
                {
        ...
        JsonElement parameterReferences, JsonElement config)
    {
        ...
        using var prepared = new SdAlignedDiagnosticInputs(inputs);
        latent = prepared.Values["latent"]; sigma = prepared.Values["sigma"];
        positive = prepared.Values["positive"]; negative = prepared.Values["negative"]; inputs = prepared.Values;
        ...
        Run(directory, reference.GetProperty("id").GetString()! + "/" + policy, "guidance", bank, parameterReferences,
            ...
            inputs, reference.GetProperty("outputs").GetProperty(policy), new(StringComparer.Ordinal), prepared,
    ...
    private static void ValidateOptions()
    {
        ...
        foreach (string variable in new[] { "COMFYSHARP_SD_NATIVE_SUITE_TRACE_DIR", "COMFYSHARP_SD_UNET_TRACE_DIR", "COMFYSHARP_SD_UNET_FINE_TRACE_DIR",
    ...
    private static void Run(string directory, string id, string kind, UnetWeightSet bank, JsonElement expectedParameters,
        ...
        Dictionary<string, HostCapture> captures, SdAlignedDiagnosticInputs prepared, Func<int, Tensor> forward, string[] sequence)
    {
    ...
    private static ParameterEvidence Parameters(UnetWeightSet bank, JsonElement references)
    {
        ...
        foreach (var item in references.EnumerateArray())
        {
            ...
            var parameterLayout = SdAlignedDiagnosticInputs.Layout(value);
            layouts.Add(name, new { shape = value.shape, stride = value.stride(), aligned64 = parameterLayout.aligned64,
                storageOffset = parameterLayout.storageOffset, addressModulo64 = parameterLayout.addressModulo64, dtype = "float32" });
        }
    ...
    private sealed class HostCapture(long[] expectedShape)
    {
        ...
        internal void Capture(Tensor value)
        {
            ...
            var layout = SdAlignedDiagnosticInputs.Layout(value);
            addressModulo64 = layout.addressModulo64; storageOffset = layout.storageOffset;
            seen = true;
        }
        internal object Record()
        {
            Assert.True(seen, "Required diagnostic boundary missing.");
            return new { shape = expectedShape, stride, aligned64, addressModulo64, storageOffset, dtype = "float32",
