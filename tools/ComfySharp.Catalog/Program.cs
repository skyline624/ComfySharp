...
try
{
    ...
    var registry = NodeRegistry.Describe(TensorNodes.CreateRegistry().Nodes.Select(n => n.Schema)
        .Concat(ImageFileNodes.Schemas).Concat(Sd15Nodes.Schemas([])).Concat(LoraNodes.Schemas([])).Append(ImageInputNodes.Describe([])).Append(SaveLoraNode.Description).Append(LoraModelLoaderNode.Description));
